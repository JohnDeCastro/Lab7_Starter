describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      if (!product.price || product.price.toString().length === 0) allArePopulated = false;
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
     ...
     //get shadow root of product
     const shadowRoot = await productItem.getProperty('shadowRoot');

     //query button inside shadow root
     ...
     //check change to "remove from cart"
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    //after all have been clicked, get count on cart
